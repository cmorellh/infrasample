variable "infrasample_private_subnets" {
  description = "Private Subnets ID for RDS DB"
  type        = list(any)
}

variable "infrasample_private_subnets_cidrs" {
  description = "Private Subnet CIDRs for RDS DB"
  type        = list(any)
}

variable "infrasample_vpc_id" {
  description = "VPC Id"
  type        = string
  validation {
    condition     = length(var.infrasample_vpc_id) > 4 && substr(var.infrasample_vpc_id, 0, 4) == "vpc-"
...
variable "db_az" {
  ...
  validation {
    ...
    error_message = "DB Avalability Zone must not be empty."
  }
}
